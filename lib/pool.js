...
Pool.prototype.discard = function(conn, callback) {
  var self = this;

  debug("discard pool:", self._pool.length, "reserved:", self._reserved.length);
...
Pool.prototype.purge = function(callback) {
  ...
  asyncjs.map(self._pool.concat(self._reserved),
      function(conn, done) {
        if (typeof conn === 'object') {
          conn.conn.isClosed(function(err, closed) {
            debug("connection is closed: " + closed);
            if (! closed) {
      ...
      function(err, result) {
        if(err) {
          ...
          if(self._driver) {
            debug("deregistering driver");
            dm.deregisterDriver(self._driver, function (err) {
              if (err) {
                callback(err);
              } else {
                self._driver = null;
